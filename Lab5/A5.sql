...
-- With existing indexes(the automatically created ones + nonclustered index on b2): 0.135151  = cu index de la b)
-- When adding a nonclustered index on a3 to the existing indexes: 0.126723 = index b) + index a3 nonclustered
-- Without the nonclustered index on b2 and the nonclustered index on a3: 0.157526 = fara index de la b) si a3
-- Automatically created indexes + nonclustered index on b2 + nonclustered index on a3 + nonclustered index on (aid, bid) from Tc: 0.125982 = toate index apar
